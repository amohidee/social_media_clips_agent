...
var YoutubeTranscript = class {
  ...
  static parseTranscriptXml(xml, lang) {
    ...
    let match2;
    while ((match2 = pRegex.exec(xml)) !== null) {
      const startMs = parseInt(match2[1], 10);
      const durMs = parseInt(match2[2], 10);
      const inner = match2[3];
      let text = "";
      const sRegex = /<s[^>]*>([^<]*)<\/s>/g;
      let sMatch;
...
async function callClaude(apiKey, prompt, useWebSearch = false) {
  const headers = {
    "Content-Type": "application/json",
    "x-api-key": apiKey,
...
function chunkTranscript(segments, chunkSize = 300) {
  const chunks = [];
  let words = [];
  let startTime = 0;
  let endTime = 0;
...
__name(chunkTranscript, "chunkTranscript");
...
app.post("/api/discovery/run", async (c) => {
  ...
  const { message = "historical Muslim TV shows and movies" } = await c.req.json();
...
[{"title_en":"...","title_ar":null,"title_ur":null,"title_tr":null,"title_translation":null,"type":"movie|tv_series|youtube_show|documentary|unknown","language":"Arabic|English|Urdu|Turkish|Persian|French|Other","year":2024,"description":"...","tags":["tag1"],"source_urls":["https://..."],"islamic_finance_relevance":"high|medium|low|none","notes":""}]`;
  try {
    const raw2 = await callClaude(apiKey, prompt, true);
    const result = extractJsonArray(raw2);
    if (result) return c.json({ text: JSON.stringify(result) });
    const raw22 = await callClaude(apiKey, `Convert this to a JSON array. Return ONLY the raw JSON array starting with [ and ending with ]:

${raw2}`, false);
    const result2 = extractJsonArray(raw22);
    if (!result2) return c.json({ error: `Could not parse response. Preview: ${raw2.slice(0, 300)}` }, 500);
...
app.post("/api/library", async (c) => {
  ...
  const insertStmts = entries.map(
    (e) => c.env.DB.prepare(
      ...
      e.islamic_finance_relevance,
      e.status,
      e.source_prompt ?? null,
      e.notes ?? null,
      e.youtube_url ?? null,
      e.discovered_at,
      e.updated_at ?? null
...
app.post("/api/transcripts/fetch", async (c) => {
  const { media_id, youtube_url, title = "" } = await c.req.json();
  ...
  try {
    ...
    const chunkStmts = chunks.map(
      (chunk, i) => c.env.DB.prepare(
...
app.post("/api/embeddings/compute", async (c) => {
  ...
  for (const row of results) {
    const embedding = getEmbedding(row.text);
    await c.env.DB.prepare("UPDATE chunks SET embedding = ? WHERE id = ?").bind(JSON.stringify(embedding), row.id).run();
  }
  return c.json({ ok: true, chunks_embedded: results.length });
});
app.post("/api/search/moments", async (c) => {
  const { query = "Islamic finance", top_k = 20, min_score = 0.05 } = await c.req.json();
  ...
  const scored = results.map((chunk) => ({
  ...
  })).filter(({ score }) => score >= min_score).sort((a, b) => b.score - a.score).slice(0, top_k).map(({ chunk, score }) => ({
    chunk_id: chunk.id,
    ...
    similarity_score: Math.round(score * 1e4) / 1e4,
...
${JSON.stringify(excerpts)}`,
      ...
      const explanations = extractJsonArray(raw2);
      if (explanations) {
        explanations.slice(0, 5).forEach((exp, i) => {
          scored[i].explanation = exp;
        });
      }
    } catch {
    }
...
app.get("/api/status", async (c) => {
  const [lib, tr, ch, approved, transcribed, embedded] = await Promise.all([
    c.env.DB.prepare("SELECT COUNT(*) as n FROM media_library").first(),
    c.env.DB.prepare("SELECT COUNT(*) as n FROM transcripts").first(),
    c.env.DB.prepare("SELECT COUNT(*) as n FROM chunks").first(),
    c.env.DB.prepare("SELECT COUNT(*) as n FROM media_library WHERE status='Approved'").first(),
    c.env.DB.prepare("SELECT COUNT(*) as n FROM transcripts WHERE status='Transcribed'").first(),
    c.env.DB.prepare("SELECT COUNT(*) as n FROM chunks WHERE embedding IS NOT NULL").first()
  ]);
  return c.json({
    ok: true,
...
// .wrangler/tmp/bundle-4o195k/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4o195k/middleware-loader.entry.ts
